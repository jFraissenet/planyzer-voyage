-- Upgrade proposal date columns from date to timestamptz so the UI can use the
-- same datetime picker as the rest of the app.
alter table public.event_tool_proposals
  alter column event_tool_proposal_date_start
    type timestamptz using event_tool_proposal_date_start::timestamptz;

alter table public.event_tool_proposals
  alter column event_tool_proposal_date_end
    type timestamptz using event_tool_proposal_date_end::timestamptz;

-- Recreate the RPC with the new return types (date_start, date_end now timestamptz).
