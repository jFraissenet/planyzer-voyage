-- Persist Maps deep links for vehicle departure/arrival locations so a tap
-- on the address opens the right point in Maps (lat/lng-based when picked
-- via the address autocomplete, falling back to a text query otherwise).

alter table public.event_tool_vehicles
  add column event_tool_vehicle_departure_location_url text,
  add column event_tool_vehicle_arrival_location_url text;
...
returns table (
  ...
  departure_location_url text,
  departure_date timestamptz,
  arrival_location text,
  arrival_location_url text,
  journey_type text,
...
language sql security definer stable set search_path = '' as $$
  select
    ...
    v.event_tool_vehicle_departure_location_url,
    v.event_tool_vehicle_departure_date,
    v.event_tool_vehicle_arrival_location,
    v.event_tool_vehicle_arrival_location_url,
